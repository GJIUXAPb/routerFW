...
*   **Подготовка:** Установите рабочий контейнерный рантайм: `Docker` или `Podman`. По умолчанию сборщик использует `ROUTERFW_RUNTIME=auto`.
...
> 4. При необходимости зафиксируйте рантайм: `--runtime=docker`, `--runtime=podman` или переменная `ROUTERFW_RUNTIME`.
...
### Доверенная модель профилей и пакетов
*   Файлы `profiles/*.conf` считаются **доверенным вводом**: билдер читает значения из профиля и shell-скриптов рядом с ним. Не запускайте чужие `.conf`, `hooks.sh`, патчи и пакеты без ручной проверки.
*   Если вы подключаете сторонние репозитории и ключи через `CUSTOM_REPOS` / `CUSTOM_KEYS`, вы берёте на себя доверие к этим источникам.
*   Для локальных сторонних пакетов в Image Builder проект может ослаблять проверку подписей внутри контейнера, поэтому неподписанные пакеты допустимы только когда вы осознанно доверяете их происхождению.

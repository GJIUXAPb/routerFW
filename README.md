...
*   **Фишки:** **Atomic Downloads** (защита от сбоев сети), умный кэш (повторная сборка ~30 сек), интеграция сторонних `.ipk/apk` (папка `custom_packages`).
...
*   `custom_packages/имя_профиля/` — Папка для `.ipk/apk` пакетов. Используется в **ImageBuilder** (прямое включение) и как источник для импорта в **SourceBuilder**.
...
# checksum:MD5=db46f12dd6ece04e81dff78e7190699c